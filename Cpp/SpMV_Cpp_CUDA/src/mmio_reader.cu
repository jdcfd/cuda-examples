...
int CSRMatrixReader::mm_init_csr(std::unique_ptr<sparse::CSRMatrixF>& mat)
{
    int ierr = mm_read_banner(this->f, &(this->mmtc));
    if (ierr) {
        std::cout << "Error reading Banner!" << std::endl;
    ...
    int nnz = nnz_file;
    if (this->is_symmetric) {
        nnz = nnz_file * 2 - nrows;
    }

    if (!mat) {
        mat = std::make_unique<sparse::CSRMatrixF>(nrows, ncols, nnz);
    } else {
        std::cout << "Matrix is not empty!" << std::endl;
        return -1;
    }
...
int CSRMatrixReader::mm_read_csr(std::unique_ptr<sparse::CSRMatrixF>& mat)
{
    std::vector<double> values_double(mat->nnz);
    int* indx = new int[mat->nnz];
    int ierr = mm_read_mtx_crd_data(this->f, mat->nrows, mat->ncols, this->nnz_from_file,
                                    indx, mat->h_cols, values_double.data(), this->mmtc);
    ...
    if (this->is_symmetric) {
        ...
        for (int i = 0; i < this->nnz_from_file; i++) {
            if (indx[i] != mat->h_cols[i]) {
                indx[k] = mat->h_cols[i];
                mat->h_cols[k] = indx[i];
                values_double[k] = values_double[i];
                k++;
            }
        }
    }
    ...
    thrust::stable_sort_by_key(thrust::host, begin_keys, end_keys, values_double.data(),
                               thrust::less<thrust::tuple<int, int>>());

    for (int i = 0; i < nnz; ++i) {
        mat->h_values[i] = static_cast<float>(values_double[i]);
    }
